...
int searchQuestionByLevel(Question **head, char* level, int *numberArray)
{
    Question *ptr;
    int i = 0;
    for (ptr = *head; ptr != NULL; ptr = ptr->next)
...
void xuatMotCauHoi(Question *head) //dung de check
{
    Question *p=head;
...
Question *exportQuestion(Question *head, char *str) //str la tham so truyen vao gom "EASY" "NORMAL" "HARD"
{
    Question *p = head;
...
int checkAnswer1(Question *head, char *answer) //answer la cai dap an ma client gui ve cho server
{
    Question *ptr = head;
...
int receiveQuestion(int socket, Question *buff, int size, int flags) //dung cho admin, phan them cau hoi cuar admin
{
  int n;
...
int sendQuestion(int socket, Question *buff, int size, int flags) //admin gui cau hoi
{
  int n;
