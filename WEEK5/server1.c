                    ...
                    if(checkStatus(head, recv_data) == 0){
                        ...
                        do{
							...
							if(checkAccount(head,recv_data,recv_data2) == 0){
                                ...
                                //send to client
                                strcpy(recv_data3, "Do you want to Sign out (y/n)?");
                                bytes_sent = send(conn_sock, recv_data3, strlen(recv_data3), 0); /* send to the client welcome message */
                                if (bytes_sent <= 0){
                                    printf("\nConnection closed");
                                    break;
                                ...
                                //receive from client laf dong ys signout hay ko
                                bytes_received2 = recv(conn_sock, recv_data2, BUFF_SIZE-1, 0); //blocking
                                if (bytes_received2 <= 0){
                                    printf("\nConnection closed");
                                    break;
                                    ...
                                    recv_data2[bytes_received2] = '\0';
                                    printf("\nReceive3: %s ", recv_data2);
                                }
                                if(strcmp(recv_data2,"y")==0 || strcmp(recv_data2, "Y") == 0){
                                    //send to client
                                    strcpy(recv_data3, "Goodbye");
                                    bytes_sent = send(conn_sock, recv_data3, strlen(recv_data3), 0); /* send to the client welcome message */
                                    if (bytes_sent <= 0){
                                        printf("\nConnection closed");
                                        break;
