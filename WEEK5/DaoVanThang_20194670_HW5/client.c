...
int main(){
	...
	char buff[BUFF_SIZE], buff2[BUFF_SIZE], buff3[BUFF_SIZE], buff4[BUFF_SIZE], buff5[BUFF_SIZE];
	struct sockaddr_in server_addr; /* server's address information */
	int msg_len, msg_len2, msg_len3, msg_len4, bytes_sent,bytes_sent2, bytes_sent3, bytes_received;
	...
	while(1){
        ...
        if(strcmp(buff3,"Login is successful!" ) == 0){
            ...
            //Nhan yeu cau muon sign out cua server va gui lai xem dong ys hay ko
            ...
            msg_len = strlen(buff);
            if (msg_len == 0) break;
            bytes_sent3 = send(client_sock, buff, msg_len, 0);
            if(bytes_sent3 <= 0){
